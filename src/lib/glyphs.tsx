...
type Shape = { fills?: string[]; strokes?: { d: string; w: number }[]; evenOdd?: boolean }
...
const SHAPES: Record<string, Shape> = {
  ...
  cloud: {
    fills: ['M38 100a25 25 0 0 1-3.4-49.8 31 31 0 0 1 58.7-6.7A23 23 0 0 1 90 100Z'],
  },
  terminal: {
    strokes: [
...
function GlyphSvg({ id }: { id: string }) {
  ...
  const fills = shape.evenOdd && shape.fills ? [shape.fills.join(' ')] : shape.fills

  const body = (
    <>
      {fills?.map((d, i) => (
        <path key={`f${i}`} d={d} fill={`url(#${gid}-grad)`} fillRule={shape.evenOdd ? 'evenodd' : 'nonzero'} />
      ))}
  ...
  return (
    <svg viewBox={`0 0 ${CANVAS_W} ${CANVAS_H}`} role="presentation" focusable="false">
      <defs>
        ...
        <filter id={`${gid}-sh`} x="-25%" y="-25%" width="150%" height="150%">
          <feDropShadow dx="0" dy="3" stdDeviation="4" floodColor="#1d3a04" floodOpacity="0.55" />
        </filter>
      </defs>

      <g filter={`url(#${gid}-sh)`} transform={`translate(14 ${GLYPH_TOP}) scale(${GLYPH_SCALE})`}>
        {body}
      </g>
...
export function GlyphMark({ glyph }: { glyph: GlyphId }) {
  ...
  return (
    <svg viewBox={`0 0 ${VB} ${VB}`} role="presentation" focusable="false">
      ...
      {(shape.evenOdd && shape.fills ? [shape.fills.join(' ')] : shape.fills)?.map((d, i) => (
        <path key={i} d={d} fill={`url(#${gid}-g)`} fillRule={shape.evenOdd ? 'evenodd' : 'nonzero'} />
      ))}
      {shape.strokes?.map((s, i) => (
        <path
          key={`s${i}`}
